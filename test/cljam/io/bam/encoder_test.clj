  ...
  (testing "non-array types"
    (are [?type ?values ?bytes]
         (= (map unchecked-byte ?bytes)
            (let [bb (ByteBuffer/allocate 100)]
    ...
    (are [?type ?values ?bytes]
         (= (map unchecked-byte ?bytes)
            (let [bb (ByteBuffer/allocate 100)]
              ...
              (take (.position bb) (.array bb))))
      \Z "aaaBBB0011223344@@@+++"
      [0x61 0x61 0x61 0x42 0x42 0x42 0x30 0x30 0x31 0x31 0x32 0x32 0x33 0x33 0x34 0x34 0x40 0x40 0x40 0x2b 0x2b 0x2b 0x00]
         ...
         (= (map unchecked-byte ?bytes)
            (let [bb (ByteBuffer/allocate 100)]
              (#'encoder/encode-tag-value
               bb \B (cstr/join \, (cons ?sub-type ?values)))
              (take (.position bb) (.array bb))))
      \c [0 127 -128 -1]
      [0x63                ;; c
